...
try:
    os.makedirs('templates\plots')
except OSError as e:
    pass

filename = 'owidCovidData\owid-covid-data.csv'

if not os.path.exists(filename): filename = 'owidCovidData\owid-covid-data-bak.csv'
    ...
    for country in interactive:
        try:
            ...
            func = [(weib, [60000, 14, 4, 500], [2000, 54, 4, 500]),
                    ...
                    (weib4, [60000, 14, 4, 500, 60000, 14, 60000, 14, 60000, 14],
                     [2000, 54, 4, 500, 4000, 54, 2000, 54, 2000, 54])]

            whichFunc = num_peaks[country] - 1
            times = 2
            skip = 30
            datacopy = deepcopy(data[1:training_data]);
            ...
            newdf = pd.DataFrame.from_dict({country + '-pred': newpredsave, \
                                            country + '-dates': [(start + timedelta(days=i)).strftime("%d %b %Y") for i
                                                                 in
                                                                 list(range(1, xlim))], \
            ...
            fig.add_trace(
                go.Scatter(x=dates, y=cumpredsave, name="Prediction (new cases)", marker=dict(color='#EB752C')))
            fig.add_trace(
                go.Bar(x=dates, y=cumsave, name="True data (new cases)", marker=dict(color='#EB752C'), opacity=0.7,
                       width=[1] * len(dates), hoverlabel=dict(bgcolor='#FF351C')))
            fig.add_trace(go.Scatter(x=dates, y=cumdpredsave, name="Prediction (deaths)", marker=dict(color='#2D58BE')),
                          secondary_y=True)
            fig.add_trace(
                go.Bar(x=dates, y=cumdsave, name="True data (deaths)", marker=dict(color='#2D58BE'), opacity=0.3,
                       width=[1] * len(dates), hoverlabel=dict(bgcolor='#1A22AB')), secondary_y=True)
            ...
            fig.update_yaxes(title_text="Number of total cases", gridcolor='lightgray', showline=True, linewidth=3,
                             linecolor='orange', gridwidth=1, secondary_y=False, )
            fig.update_yaxes(tickformat=',.0f', title_text="Number of total deaths", secondary_y=True,
                             gridcolor='lightgray', showline=True, linewidth=3, linecolor='blue', gridwidth=1)
            fig.write_html("templates/plots/graph.html")
            fig.data = []
